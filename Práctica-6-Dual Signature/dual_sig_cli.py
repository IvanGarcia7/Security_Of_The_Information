...
#realizo el hash del OI
OIMD = SHA256.new(OI_Str.encode("UTF-8")).digest()

#realio el hash del PO
PIMD = SHA256.new(PI_Str.encode("UTF-8")).digest()

#Concateno OI y PI
OIMD_PIMD = OIMD+PIMD

#Realizo el hash de dicha concatenacion
HASH_OIMD_PIMD = SHA256.new(OIMD_PIMD).digest()

#Firmo dicho mensaje por parte del usuario
firma_dual = RSA_Usuario.sign(HASH_OIMD_PIMD)
